...
def apply_P(P, num):
    (pid, N) = P
    p_binstr = ("{0:0%db}" % pow(2,N)).format(pid)[::-1]
    #print(p_binstr)
    return p_binstr[num]

def apply_Ps(P, num, count):
    (pid, N) = P
    num_binstr = ("{0:0%db}" % (N*count)).format(num)
    #print(num_binstr)
    binstr_out = ''
    for i in range(count):
        binstr_out += apply_P(P, int(num_binstr[i*N:i*N+N],2))
    #print(binstr_out)
    return binstr_out
...
def check_rule_trans(rule1, rule2, N):
    global Count
    Count = 0
    if rule2 == 0:
        binstr = '1'*pow(2,N)
        ...
        res = []
        pidspace = pow(2,pow(2,N))
        for pid in range(pidspace):
            #print(pid)
            if (pid == 0 or pid == pidspace-1): continue
            for i in range(pow(2, 3*N)):
                #print(i)
                temp = apply_rules(rule1, i, N)
                #print(temp)
                res1 = apply_P((pid,N), int(temp,2))
                res2 = apply_rule(rule2, int(apply_Ps((pid,N), i, 3),2))
                Count += 1
                if res1 != res2:
                    #print(res1,res2)
                    break
